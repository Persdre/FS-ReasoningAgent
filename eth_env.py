...
def get_paths(args):
    ...
    # this is the new price data
    price_dict = {'eth': 'Ethereum_31_10_2023-22_07_2024_historical_data_coinmarketcap.csv', 'btc': 'Bitcoin_31_10_2023-22_07_2024.csv', 'sol': 'Solana_31_10_2023-22_07_2024.csv'}  
    txn_dict = {'eth': 'ethereum.csv', 'btc': 'bitcoin.csv', 'sol': 'solana.csv'}
    # news_dict = {'eth': 'gnews', 'btc': 'selected_bitcoin_202301_202401', 'sol': 'selected_solana_202301_202401'}
    # the updated news dict
    news_dict = {'eth': 'selected_updated_ethereum', 'btc': 'selected_updated_bitcoin', 'sol': 'selected_updated_solana'}
    # timecol_dict = {'eth': 'snapped_at', 'btc': 'timeOpen', 'sol': 'timeOpen'}
    # the updated timecol_dict
    timecol_dict = {'eth': 'timestamp', 'btc': 'timestamp', 'sol': 'timestamp'}
    # price_timefmt_dict = {'eth': "%Y-%m-%d %H:%M:%S UTC", 'btc': "%Y-%m-%dT%H:%M:%S.%fZ", 'sol': "%Y-%m-%dT%H:%M:%S.%fZ"}
    # the updated price_timefmt_dict
    price_timefmt_dict = {'eth': "%Y-%m-%dT%H:%M:%S.%fZ", 'btc': "%Y-%m-%dT%H:%M:%S.%fZ", 'sol': "%Y-%m-%dT%H:%M:%S.%fZ"}
    # the eth format is 2023-11-01, change the txt_timefmt accordingly
    txn_timefmt_dict = {'eth': "%Y-%m-%d", 'btc': "%Y-%m-%d", 'sol': "%Y-%m-%d"}
    # txn_timefmt_dict = {'eth': "%d/%m/%y %H:%M", 'btc': "%Y-%m-%d %H:%M:%S.%f UTC", 'sol': "%Y-%m-%d %H:%M:%S.%f UTC"}
    return f'data/{price_dict[dataset]}', f'data/{txn_dict[dataset]}', f'data/{news_dict[dataset]}', timecol_dict[dataset], price_timefmt_dict[dataset], txn_timefmt_dict[dataset], dataset


class ETHTradingEnv:
    def __init__(self, args):
        price_path, txn_path, self.news_dir, self.timecol, self.price_timefmt, txn_timefmt, dataset = get_paths(args)
